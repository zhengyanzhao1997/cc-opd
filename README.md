...
Code for **Counterfactual Constraint-Conditioned On-Policy Distillation for Multi-Constraint Instruction Following** (EMNLP 2026).

Yanzhao Zheng, Yuanqiang Yu, Tianze Xu, Chao Ma, Zhentao Zhang, Jihuai Zhu, Baohua Dong, Hangcheng Zhu, and Ruohui Huang · Alibaba Group
...
Only these two methods are supported. The other baselines in the paper are **not** implemented in this release.
...
The launcher uses $\lambda=2$, clip threshold $c=5$, and all available constraints ($\rho=1$) for LOO by default. Both methods retain the experimental actor update implementation, including dual-clip PPO with `clip_ratio_c=3.0`.
