...
# To support different vLLM versions, add optional model classes lazily. Importing
# every vLLM model module at module load time can trigger heavyweight Transformers
# model imports in zero-GPU Ray control actors.
SUPPORTED_MOE_MODELS = []
_supported_moe_models_loaded = False
_SUPPORTED_MOE_MODEL_SPECS = (
    ...
    ("vllm.model_executor.models.qwen3_vl_moe", ("Qwen3MoeLLMForCausalLM",)),
...
def _load_supported_moe_models():
    global _supported_moe_models_loaded

    if _supported_moe_models_loaded:
        return SUPPORTED_MOE_MODELS

    for module_name, class_names in _SUPPORTED_MOE_MODEL_SPECS:
        try:
            module = importlib.import_module(module_name)
        except ImportError:
            continue
        except RuntimeError as exc:
            print(f"[vllm_utils] skip optional MOE model module {module_name}: {exc}")
            continue

        for class_name in class_names:
            model_cls = getattr(module, class_name, None)
            if model_cls is not None:
                SUPPORTED_MOE_MODELS.append(model_cls)

    _supported_moe_models_loaded = True
    return SUPPORTED_MOE_MODELS
...
def patch_vllm_moe_model_weight_loader(model):
    ...
    supported_moe_models = _load_supported_moe_models()
...
class TensorLoRARequest(LoRARequest):
    peft_config: dict[str, object] | None = field(default=None)
    lora_tensors: dict[str, object] | None = field(default=None)
