...
class TrajectoryCollector:
    ...
    def preprocess_single_sample(
        ...
        original_raw_prompt = gen_batch.non_tensor_batch['raw_prompt'][item]
        raw_prompt = original_raw_prompt
        data_source = gen_batch.non_tensor_batch['data_source'][item]
        apply_chat_template_kwargs = self.config.data.get("apply_chat_template_kwargs", {})
        task_type = gen_batch.non_tensor_batch.get("task_type", [None] * len(gen_batch.batch))[item]
        extra_info = gen_batch.non_tensor_batch.get("extra_info", [None] * len(gen_batch.batch))[item]
        cc_env_kwargs = gen_batch.non_tensor_batch.get("cc_env_kwargs", [None] * len(gen_batch.batch))[item]
        ...
        if isinstance(cc_env_kwargs, dict) and isinstance(cc_env_kwargs.get('question'), str):
            row_dict['cc_full_instruction'] = cc_env_kwargs['question']
        elif isinstance(original_raw_prompt, (list, np.ndarray)) and len(original_raw_prompt) > 0:
            first_message = original_raw_prompt[0]
            if isinstance(first_message, dict) and isinstance(first_message.get('content'), str):
                row_dict['cc_full_instruction'] = first_message['content']
        ...
        if cc_delta_key:
            if cc_delta_key in gen_batch.batch:
                row_dict[cc_delta_key] = gen_batch.batch[cc_delta_key][item]
            elif cc_delta_key in gen_batch.non_tensor_batch:
                row_dict[cc_delta_key] = gen_batch.non_tensor_batch[cc_delta_key][item]
    ...
    def preprocess_batch(
        ...
        Parameters:
            ...
            obs (Dict): Environment observation dictionary
                ...
                - 'anchor' (None or Any): Optional observation metadata.
    ...
    def vanilla_multi_turn_loop(
        ...
        # Initial observations from the environment. Keep a copy of env_kwargs for
        # online CC-OPD prompt reconstruction; MathEnv consumes the same question
        # string that later defines full-vs-leave-one-out teacher contexts.
        env_kwargs = gen_batch.non_tensor_batch.get('env_kwargs', None)
        if env_kwargs is not None:
            gen_batch.non_tensor_batch['cc_env_kwargs'] = env_kwargs
    ...
    def multi_turn_loop(
        ...
        """
        Run the rollout loop for OPD or LOO CC-OPD.

        Args:
            gen_batch (DataProto): Initial prompt batch.
            actor_rollout_wg: Actor model workers.
            envs (EnvironmentManagerBase): Environment manager for interaction.
            is_train (bool): Whether to draw the configured number of responses per prompt.
